...
class Template_Compiler_
{
    function _compile_template($tpl, $template_path, $compile_base, $compile_header)
    {
        ...
        $this->all_functions = array_merge($functions['internal'], $functions['user'], [
                'isset',
                'empty',
                'eval',
                'list',
                    ...
                    if (is_array($result)) {
                        ...
                        if ($this->mark_php) {
                            ...
                            if ($result[1]) {
                                ...
                                $this->func_list[$result[1]] = [];
                                $this->obj_list[$result[1]]  = [];
                        ...
                        $erlist[] = [
                            htmlspecialchars($this->_split[$this->_index - 1]),
                            $this->nl_cnt,
                        ];
    ...
    function _compile_statement($statement)
    {
        ...
        if ($match[1]) {
            switch ($match[2]) {
                case '#':
                    return preg_match('/^[A-Z_a-z\x7f-\xff][\w\x7f-\xff]*$/', $src) ? [
                        16,
                        substr($statement, 1),
                    ] : 0;
                case '+':
                    return !strlen($src) || preg_match('/^[A-Z_a-z\x7f-\xff][\w\x7f-\xff]*$/', $src) ? [
                        16,
                        substr($statement, 1),
                    ] : 0;
                case '/':
                ...
                case '?':
                    return $this->_compile_branch($src, 1, 0) !== 0 ? [
                        16,
                        substr($statement, 1),
                    ] : 0;
                case ':':
                    return !strlen($src) || $this->_compile_branch($src, 1, 1) !== 0 ? [
                        16,
                        substr($statement, 1),
                    ] : 0;
                case '' :
                    return $this->_compile_expression($src, 1, 1) !== 0 ? [
                        16,
                        substr($statement, 1),
                    ] : 0;
                default :
                    return $this->_compile_expression($src, 1, 0) !== 0 ? [
                        16,
                        substr($statement, 1),
                    ] : 0; // = @ :?
            }
        ...
        switch ($match[2]) {
            case ''  :
                return (($xpr = $this->_compile_expression($src, 0, 1)) === 0) ? 0 : [
                    1,
                    'echo ' . $xpr,
                ];
            case '=' :
                return (($xpr = $this->_compile_expression($src, 0, 0)) === 0) ? 0 : [
                    1,
                    'echo ' . $xpr,
                ];
            case ':?':
            ...
            case '@' :
                ...
                if ($xpr) {
                    ...
                    return [
                        2,
                        'if(is_array($TPL_R' . $d . '=' . $xpr . ')&&!empty($TPL_R' . $d . ')){',
                    ];
                }
    ...
    function _compile_expression($expression, $escape = 0, $no_directive = 0)
    {
        ...
        for ($xpr = '', $i = 0; strlen($expression); $expression = substr($expression, strlen($m[0])), $i++) {    //
            ...
            if (!empty($m[10])) {    // (.+)
                    ...
                    case '(' :
                        if ($var_state[0]) {
                            ...
                            if (in_array($func, $this->func_plugins)) {
                                ...
                                if ($this->safe_mode) {
                                    in_array($func, $this->safe_mode_functions) or $this->exp_error[] = [
                                        5,
                                        $m[2],
                                    ];
                                } else {
                                    in_array($func, $this->all_functions) or $this->exp_error[] = [
                                        7,
                                        $m[2],
                                    ];
                                }
                ...
                $xpr             = preg_replace('/\+$/', '.', $xpr) . strtr($m[7], [
                        '``'  => '`',
                        '{`'  => '{',
                        '`}'  => '}',
                        '<?`' => '<?',
