...
class Template_
{
    ...
    {
        $this->compile_dir  = PATH_TPL_COMPILE;
        $this->template_dir = rtrim(PATH_VIEWS, '/');
        $this->cache_dir    = PATH_TPL_CACHE;
    }
    ...
    {
        if ($n === 1) {
            $this->var_[$this->_current_scope][$id] = is_array($arg) ? $arg : array_pad([], $arg, 1);
        } else {
            $this->{'b' . --$n}[count($this->{'b' . $n}) - 1][$id] = is_array($arg) ? $arg : array_pad([], $arg, 1);
        }
    }

    function pushSetLoop($id, $arg, $n = 2)
    {
        $this->{'b' . --$n}[][$id] = is_array($arg) ? $arg : array_pad([], $arg, 1);
    }
}
